...
import de.monticore.lang.sysml4verification._lsp.LanguageServerCLI;
...
public class UpperCaseBlockNameCoCoCodeActionProvider extends CoCoCodeActionProvider {
  ...
  public List<CodeAction> createFixesFor(TextDocumentItem document, Diagnostic diagnostic) {
    ...
    traverse((ISysML4VerificationScope) ast.getEnclosingScope(), symbol -> {
      if (symbol == null)
        return;
  ...
  private static void traverse(ISysML4VerificationScope scope, Consumer<SysMLTypeSymbol> f) {
    if (scope != null) {
