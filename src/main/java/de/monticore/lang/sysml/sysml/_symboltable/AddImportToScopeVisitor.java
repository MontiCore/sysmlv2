...
public class AddImportToScopeVisitor implements SysMLInheritanceVisitor {
  ...
  public void visit(ASTAliasPackagedDefinitionMember node) {
    ...
    resolveImports(node.getQualifiedName(), node.getEnclosingScope().getEnclosingScope().getEnclosingScope(), node.getEnclosingScope(), false);
    //TODO remove
  ...
  public void visit(ASTImportUnitStd node) {
    outForTesting("Checking node");
    resolveImports(node.getQualifiedName(), node.getEnclosingScope().getEnclosingScope().getEnclosingScope(), node.getEnclosingScope(), node.isStar());
  }

  public void resolveImports(ASTQualifiedName qualifiedName, ISysMLNamesBasisScope scopeToSearchIn2, ISysMLNamesBasisScope currentEnclosingScope, boolean starImport) {
    ...
    if (names.size() == 0) {
      Log.error("Internal error in " + this.getClass().getName() + ". A Qualified Name should always start with a " + "Name");
    }
    ...
    if (resolvedTypes.size() == 0) {
      ...
      if (starImport) {
        ...
        warnings.add("Importing a package without a star (e.g.\"::*\") will have no effect. " + "If this Statement imports something else then a scope, this has no effect.");//TODO CoCo
      }
    }
